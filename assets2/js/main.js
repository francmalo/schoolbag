(function ($) {
	...
	////////////////////////////////////////////////////
	// 18. Mobile Menu Js
	$('#mobile-menu').mean2menu({
		mean2MenuContainer: '.mobile-menu',
		mean2ScreenWidth: "1199",
		mean2Expand: ['<i class="fal fa-plus"></i>'],
	});
    ...
    $('.user-menu-toggle').on('click', function(e) {
        e.preventDefault();
        $(this).next('.user-menu-dropdown').slideToggle(200);
    });
...
$(document).ready(function() {
    ...
    function updateCartCount(count) {
        $('.header__info-cart span').text(count);
    }

  // Update cart sidebar
    function updateCartSidebar(cartHtml, cartTotal) {
        $('.tpcart__product-list ul').html(cartHtml);
        $('.heilight-price').text(parseFloat(cartTotal).toLocaleString() + ' Ksh');
    }
 $('#add-to-cart').submit(function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(result.status === 'success') {
                    ...
                    updateCartSidebar(result.cartHtml, result.cartTotal);
    ...
    $(document).on('click', '.remove-cart-item', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(result.status === 'success') {
                    updateCartCount(result.cartCount);
                    updateCartSidebar(result.cartItems, result.cartTotal);
                    showToast(result.message, 'success');
                } else {
                    showToast(result.message, 'error');
                }
    ...
    // Open cart sidebar
    $('.tp-cart-toggle').on('click', function(e) {
        e.preventDefault();
        $('.tpcartinfo').addClass('opened');
        $('.cartbody-overlay').addClass('opened');
    });

    // Close cart sidebar
    $('.tpcart__close, .cartbody-overlay').on('click', function() {
        $('.tpcartinfo').removeClass('opened');
        $('.cartbody-overlay').removeClass('opened');
    });
    ...
    function showToast(message, type) {
        ...
        Toastify({
            ...
            className: `custom-toast custom-toast-${type} toastify-on-click`,
            stopOnFocus: true,
            escapeMarkup: false
        }).showToast();
    }
